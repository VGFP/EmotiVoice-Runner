...
def download_large_file_from_hugging_face(file_name: str, hugging_face_url: str, local_directory: str, chunk_size: int = 1024*1024) -> None:
    """
...
'''
def run_iference(logdir: str, config_folder: str, checkpoint: str, processed_text: str, output_path: str):


    config = Config()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    root_path = os.path.join(config.output_directory, logdir)
    ...
    for file in files:
        ...
        if os.path.exists(output_path + f"1{file}/"):
            r = glob.glob(output_path + f"/{file}/*")
            for j in r:
                os.remove(j)
        texts = []
        prompts = []
        ...
        concatenated_audio = np.concatenate(all_audio_samples)
        if not os.path.exists(output_path):
            os.makedirs(output_path, exist_ok=True)

        # Write the concatenated audio to a file
        sf.write(os.path.join(output_path, output_file), data=concatenated_audio, samplerate=config.sampling_rate)
'''

def initialize_inference(logdir: str, checkpoint: str) -> Tuple[torch.device, nn.Module, nn.Module, PreTrainedTokenizer, Dict[str, int], Dict[str, int]]: 
