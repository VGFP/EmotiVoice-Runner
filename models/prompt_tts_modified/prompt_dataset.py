...
class Dataset_PromptTTS(torch.utils.data.Dataset):
    def __init__(self, data_path, config, style_encoder):
        ...
        self.config = config
        self.pitch_extractor = feats.Pitch(
            sr=config.sampling_rate,
            hop_length=config.hop_length,
            pitch_min=config.pitch_min,
            pitch_max=config.pitch_max,
        )
        self.energy_extractor = feats.Energy(
            sr=config.sampling_rate,
...
class Dataset_Prompt_Pretrain(torch.utils.data.Dataset):
    ...
    def TextMelCollate(self, data):
        prompt = self.tokenizer.batch_encode_plus(
            [x["prompt"] for x in data], return_tensors="pt", padding=True
        )  # return a dict that has "input_ids", "token_type_ids", "attention_mask"
