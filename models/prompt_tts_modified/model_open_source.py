...
class PromptTTS(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.embed_projection1 = nn.Linear(
            config.model.encoder_n_hidden * 2 + config.model.bert_embedding * 2, config.model.encoder_n_hidden
        )
    ...
    def forward(
        ...
        x = torch.concat(
            [
                x,
                speaker_embedding.unsqueeze(1).expand(B, T, -1),
                inputs_style_embedding.unsqueeze(1).expand(B, T, -1),
                inputs_content_embedding.unsqueeze(1).expand(B, T, -1),
            ],
            dim=-1,
        )
        ...
        if mel_targets is not None:
            log_p_attn = self.alignment_module(
                text=x,
                feats=mel_targets.transpose(1, 2),
                text_lengths=input_lengths,
                feats_lengths=output_lengths,
                x_masks=src_mask,
            )
